...
同一个 Agent 可以同时连接多个渠道，消息在各平台间独立处理。用户在飞书发送的消息不会出现在 Discord 中，反之亦然。
...
nexu 采用桌面端本地部署模式。启动客户端、连接渠道，Agent 即刻就绪，可以开始接收和处理消息。

无需购买服务器、无需配置容器编排，所有运行时管理由桌面客户端自动完成。

## 工作区（Workspace）

工作区是 nexu 中所有配置的归属单元，关联了 Agent、渠道、模型和技能。一个工作区即一套完整的运行环境。

你在 nexu 客户端中的所有操作——连接渠道、配置模型、安装技能——都作用于当前工作区。
