...
async function api(path, retries = 4) {
  for (let i = 0; i < retries; i++) {
    ...
    if (res.status === 202) { console.log(`  ${path} → 202, retry ${i+1}...`); await new Promise(r => setTimeout(r, 4000)); continue; }
...
async function searchCount(q) {
  const res = await fetch(`https://api.github.com/search/issues?q=${encodeURIComponent(q)}&per_page=1`, { headers });
  return res.ok ? ((await res.json()).total_count ?? 0) : 0;
}
function pct(cur, prev) { return prev === 0 ? (cur > 0 ? 100 : 0) : ((cur - prev) / prev * 100); }

async function fetchData() {
  const now = new Date(), d30 = new Date(now - 30*86400000), d60 = new Date(now - 60*86400000);
  const iso30 = d30.toISOString().slice(0,10), iso60 = d60.toISOString().slice(0,10), isoNow = now.toISOString().slice(0,10);

  const [ioCur, ioPrev, icCur, icPrev, prCur, prPrev, repoInfo, contributors] = await Promise.all([
    searchCount(`repo:${OWNER}/${REPO} is:issue created:${iso30}..${isoNow}`),
    searchCount(`repo:${OWNER}/${REPO} is:issue created:${iso60}..${iso30}`),
    searchCount(`repo:${OWNER}/${REPO} is:issue is:closed closed:${iso30}..${isoNow}`),
    searchCount(`repo:${OWNER}/${REPO} is:issue is:closed closed:${iso60}..${iso30}`),
  ...
  const commitActivity = await api(`/repos/${OWNER}/${REPO}/stats/commit_activity`);

  // Fetch recent issues & PRs via list API for accurate daily counts
  async function listItems(type, params = "") {
    const items = [];
    for (let page = 1; page <= 3; page++) {
      const data = await api(`/repos/${OWNER}/${REPO}/${type}?state=all&per_page=100&page=${page}&sort=created&direction=desc${params}`).catch(() => []);
  ...
  const days = 10;
  const dIO = new Array(days).fill(0), dIC = new Array(days).fill(0);
  const dPO = new Array(days).fill(0), dPC = new Array(days).fill(0);
  ...
  if (Array.isArray(contributors)) {
    for (const c of contributors) {
      ...
      if (commits > 0) topC.push({ login: c.author?.login ?? "unknown", commits, weeks: weeks.map(w => w.c) });
    }
  ...
  return {
    totalContrib, heatmap,
    ratio, ratioDelta: ratio - ratioPrev, ratioPct: pct(ratio, ratioPrev),
    prCur, prDelta: prCur - prPrev, prPct: pct(prCur, prPrev),
    pushCur, pushDelta: pushCur - pushPrev, pushPct: pct(pushCur, pushPrev),
    dIO, dIC, dPO, dPC, dPush, topC,
  };
}
...
function render(d) {
  const W = 960, pad = 32, cw = W - pad * 2; // content width 896
  const cardGap = 14, cardW = Math.floor((cw - cardGap * 2) / 3); // ~289

  // Heatmap colors (pink/magenta like reference)
  const hmMax = Math.max(...d.heatmap, 1);
  const hmColor = v => v === 0 ? "#ebedf0" : v/hmMax <= 0.25 ? "#ffb3c6" : v/hmMax <= 0.5 ? "#ff7096" : v/hmMax <= 0.75 ? "#e8457a" : "#c9184a";
  ...
  function bars(d1, d2, c1, c2, title, titleIcon, legendItems) {
    const n = d1.length, ch = 90, minH = 4;
  ...
  function miniHM(weeks) {
    ...
    for (let c = 0; c < cols; c++) {
      for (let r = 0; r < 3; r++) {
        ...
        const color = v > mx*0.5 ? "#216e39" : v > mx*0.2 ? "#40c463" : v > mx*0.02 ? "#9be9a8" : "#ebedf0";
...
<g transform="translate(${pad}, 216)">
  <g transform="translate(0, 0)">${bars(d.dIO, d.dIC, "#54aeff", "#0969da", "Issues", issueIcon, [{color:"#54aeff",label:"Opened"},{color:"#0969da",label:"Closed"}])}</g>
  <g transform="translate(${cardW + cardGap}, 0)">${bars(d.dPO, d.dPC, "#e085d0", "#8250df", "Pull Requests", prIcon, [{color:"#e085d0",label:"Opened"},{color:"#8250df",label:"Closed"}])}</g>
...
async function main() {
  ...
  console.log(`  Contributions: ${data.totalContrib}, PRs: ${data.prCur}, Pushes: ${data.pushCur}`);
  console.log(`  Issue ratio: ${data.ratio.toFixed(2)} (Δ${data.ratioDelta.toFixed(2)})`);
