...
Nexu Desktop (Controller) 和 OpenClaw (Gateway) 各自独立进行健康监控，缺乏共享上下文和协调机制：

- **Desktop** 每 5s 对 OpenClaw 的 `/health` 端点发起 HTTP 探针。连续失败 12 次（约 108s）后判定为"楔死"（wedge）。但它无法区分真正的死锁与配置重载、channel 重连或 auth 刷新——这些都会导致瞬态探针失败。

- **OpenClaw** 拥有完善的内部监控体系（channel 健康检测、session 卡死检测、tool 死循环熔断、auth 过期检查），能自愈大多数状况。但它检测不了自身进程级别的楔死，也没有在内部恢复失败时向 Desktop 升级的通道。

- **结果**：良性操作时产生误报 wedge 告警，OpenClaw 到 Desktop 没有升级路径，缺乏协调恢复流程，也没有统一的远程上报和用户交互通道。

---

## 2. 目标

1. **分层异常检测** — OpenClaw 处理应用内部故障；Desktop 处理进程外部故障。
2. **分层自愈** — OpenClaw 在内部执行重试/重启/熔断；Desktop 仅在 OpenClaw 无法恢复时进行进程级干预。
3. **语义化协调** — 用正式协议替代仅依赖探针的隐式监控，消除良性瞬态状态导致的误报。
4. **统一升级路径** — 检测 → 自愈 → 升级 → Desktop 干预 → 远程上报。
5. **用户可触达的修复入口** — IM 命令（`/diagnose`、`/fix`）为用户提供自助诊断和修复入口。
