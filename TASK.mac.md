...
- 当前分支近期改动很大一部分就在 `scripts/dev`、桌面运行时平台抽象、controller/web/desktop 的协同启动链路
...
3. 至少执行一次 stop -> restart -> stop，确认 `scripts/dev` 编排是稳定的

如果 Phase 0 没过：

- **暂停 packaged 验证**
- 优先排查 `scripts/dev`、桌面冷启动、controller/web readiness、OpenClaw 启动链路
