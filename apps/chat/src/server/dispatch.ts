...
const openclawGatewayToken = process.env.OPENCLAW_GATEWAY_TOKEN ?? "gw-secret-token";
...
export async function dispatchSessionChatTurn(input: {
  ...
  try {
    ...
    if (!response.ok) {
      const dispatchError = json.error?.message ?? `OpenClaw dispatch failed with status ${response.status}.`;
    ...
    const assistantText = extractAssistantText(json) ?? "OpenClaw completed without assistant text.";
    ...
    await updateSessionChatMessageTraceState(userMessage.id, "dispatch-succeeded");
    ...
    const dispatchError = error instanceof Error ? error.message : "Unknown dispatch failure.";
    await updateSessionChatMessageTraceState(userMessage.id, "dispatch-failed");
    await updateSessionChatThreadStatus(input.threadId, "dispatch-failed");
    await addSessionChatTrace({
      messageId: userMessage.id,
