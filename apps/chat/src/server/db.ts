...
const databaseUrl =
  process.env.DATABASE_URL ?? "postgresql://postgres:postgres@127.0.0.1:50822/postgres?sslmode=disable";
const migrationsDir = process.env.SESSION_CHAT_MIGRATIONS_DIR ?? join(process.cwd(), "migrations");
...
const pool = globalThis.__sessionChatPool ?? new Pool({ connectionString: databaseUrl });
...
export async function listSessionChatMessages(threadId: string): Promise<SessionChatMessage[]> {
...
export async function listSessionChatTraces(messageId: string): Promise<SessionChatTrace[]> {
...
export async function createSessionChatThread(title: string): Promise<SessionChatThread> {
...
export async function createSessionChatMessage(input: {
  ...
  const rows = await querySessionChat<SessionChatMessageRow>(
    ...
    [messageId, input.threadId, input.role.trim() || "user", trimmedBody, correlationId, now]
  );
...
async function recordSessionChatTrace(input: {
  ...
  await querySessionChat(
    ...
    [randomUUID(), input.messageId, input.correlationId, input.stage, input.source, input.detail, input.createdAt]
  );
}
...
export async function appendSessionChatMessage(input: {
  ...
  const rows = await querySessionChat<SessionChatMessageRow>(
    `insert into session_chat_messages (id, thread_id, role, body, correlation_id, trace_state, created_at)
     values ($1, $2, $3, $4, $5, $6, $7)
     returning id, thread_id, role, body, correlation_id, trace_state, created_at`,
    [randomUUID(), input.threadId, input.role, input.body, input.correlationId, input.traceState, createdAt]
  );

  return mapMessage(rows[0]);
}

export async function updateSessionChatMessageTraceState(messageId: string, traceState: string): Promise<void> {
  await querySessionChat(`update session_chat_messages set trace_state = $2 where id = $1`, [messageId, traceState]);
}

export async function updateSessionChatThreadStatus(threadId: string, status: string): Promise<void> {
  await querySessionChat(
    `update session_chat_threads
     set status = $2,
         updated_at = $3
