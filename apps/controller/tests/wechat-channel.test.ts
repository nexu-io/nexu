...
describe("WeChat connect/disconnect lifecycle", () => {
  ...
  it("connectWechat returns immediately without waiting for readiness", async () => {
  ...
  it("connectWechat does not rollback on slow runtime startup", async () => {
    // Even if readiness would fail, connectWechat should not disconnect.
    // Status will be shown via live-status polling on the frontend.
    ...
    // Should succeed — no disconnect, no rollback
    expect(channel.channelType).toBe("wechat");
