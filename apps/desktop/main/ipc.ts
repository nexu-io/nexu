...
function assertValidChannel(channel: string): asserts channel is keyof HostInvokePayloadMap {
...
export function registerIpcHandlers(orchestrator: RuntimeOrchestrator): void {
  ipcMain.handle("host:invoke", async (_event, channel: string, payload: unknown) => {
    ...
    switch (channel) {
      ...
      case "runtime:start-unit": {
        const typedPayload = payload as HostInvokePayloadMap["runtime:start-unit"];
        return orchestrator.startOne(typedPayload.id);
      }

      case "runtime:stop-unit": {
        const typedPayload = payload as HostInvokePayloadMap["runtime:stop-unit"];
      ...
      case "shell:open-external": {
        const typedPayload = payload as HostInvokePayloadMap["shell:open-external"];
